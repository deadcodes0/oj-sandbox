...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question>
    ...
    public void validQuestion(Question question, boolean add) {
        ...
        if (StringUtils.isNotBlank(judgeConfig)) {
            ...
            if (CompareMode.SPJ.getValue().equals(judgeConfigObj.getCompareMode())
                    && StringUtils.isAnyBlank(question.getSpjCode(), question.getSpjLanguage())) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "特判题目必须配置特判代码和语言");
            }
        }
        if (StringUtils.isNotBlank(question.getSpjCode()) && question.getSpjCode().length() > 8192) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "特判代码过长");
        }
        if (StringUtils.isNotBlank(question.getSpjLanguage()) && question.getSpjLanguage().length() > 128) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "特判语言过长");
        }
    }
